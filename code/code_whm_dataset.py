...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, imlist_file, num_classes, spacing, crop_size, default_values, sampling_method,
        ...
        :param spacing: the resolution, e.g., [1, 1]
        :param crop_size: crop size, e.g., [96, 96]
        ...
        #self.spacing = np.append(self.spacing, self.D)

        self.crop_size = np.array(crop_size, dtype=np.int32)
        assert self.crop_size.size == 2, 'only 2-element of crop size is supported'
        # self.crop_size = np.append(self.crop_size, self.D)

        assert sampling_method in ('GLOBAL', 'MASK', 'CENTER', 'MIX', 'RANDOM'), 'sampling_method must either be GLOBAL or MASK'
    ...
    def random_sample(self, seg):
        slices = seg.GetSize()[2]
        return random.choice(range(slices))
    ...
    def global_sample(self, seg):
        ...
        sp[2] = np.random.uniform(0, im_size[2])
    ...
    def center_sample(self, seg):
        im_size = np.array(seg.GetSize())[:2]
    ...
    def generate_center(self, seg, method):
        if method == 'MASK':
            ...
            return center
        if method == 'GLOBAL':
            slice_center = self.random_sample(seg)
    ...
    def __getitem__(self, index):
        ...
        if self.sampling_method == 'RANDOM':
            sampling_method = random.choice(['MASK', 'MIX', 'GLOBAL', 'CENTER'])
        else:
        ...
        spacing = np.append(self.spacing, ori_spacing[2])
